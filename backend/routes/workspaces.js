...
router.post('/workspaces/join', authenticate, async (req, res) => {
	...
	if (!join_code) return res.status(400).json({ message: 'join_code required' });
	try {
		const workspace = await Workspace.findOne({ where: { join_code } });
		if (!workspace) return res.status(404).json({ message: 'Workspace not found' });
		if (workspace.join_code_expires_at && workspace.join_code_expires_at < new Date()) {
...
router.post('/workspaces/:workspaceId/rotate-join-code', authenticate, requireWorkspaceRole(['admin']), async (req, res) => {
	...
	try {
		...
		if (!workspace) return res.status(404).json({ message: 'Workspace not found' });
		await workspace.update({ join_code: generateJoinCode(), join_code_expires_at: null });
