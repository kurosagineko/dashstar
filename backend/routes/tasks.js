...
import { Task, TaskAssignment, User, WorkspaceUser, Team } from '../models/index.js';
...
router.post('/admin/tasks', authenticate, requireWorkspaceRole(['admin']), async (req, res) => {
	...
	try {
		const team = await Team.findOne({ where: { id: team_id, workspace_id: req.workspaceId, deleted_at: null } });
...
router.post('/tasks/:taskId/claim', authenticate, requireWorkspaceMember, async (req, res) => {
	const { taskId } = req.params;
	try {
		const task = await Task.findOne({ where: { id: taskId, workspace_id: req.workspaceId, deleted_at: null } });
		if (!task) return res.status(404).json({ message: 'Task not found' });
		...
		if (task.team_id && user.team_id !== task.team_id) {
			return res.status(403).json({ message: 'Task is not assigned to your team' });
		}

		const existing = await TaskAssignment.findOne({ where: { task_id: task.id, user_id: req.user.id } });
		if (existing) {
		...
		const assignment = await TaskAssignment.create({ task_id: task.id, user_id: req.user.id, status: 'in_progress' });
...
router.post('/tasks/:taskId/complete', authenticate, requireWorkspaceMember, async (req, res) => {
	const { taskId } = req.params;
	try {
		const task = await Task.findOne({ where: { id: taskId, workspace_id: req.workspaceId, deleted_at: null } });
		if (!task) return res.status(404).json({ message: 'Task not found' });
		...
		const assignment = await TaskAssignment.findOne({ where: { task_id: task.id, user_id: req.user.id } });
		if (!assignment) return res.status(404).json({ message: 'Task not claimed' });
		if (assignment.status === 'completed') return res.status(400).json({ message: 'Task already completed' });
...
router.post('/admin/tasks/:taskId/assign', authenticate, requireWorkspaceRole(['admin']), async (req, res) => {
	...
	try {
		...
		if (task.team_id && user.team_id !== task.team_id) {
			return res.status(403).json({ message: 'User is not in the task team' });
		}
