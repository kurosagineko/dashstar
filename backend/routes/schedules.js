...
router.post('/admin/schedules', authenticate, requireWorkspaceRole(['owner', 'admin']), async (req, res) => {
	const { title, description, start_at, end_at, status = 'planned', target_team_id, target_user_id } = req.body;
	if (!title || !start_at) return res.status(400).json({ message: 'title and start_at required' });
	try {
		if (target_team_id) {
